...
def convolution_block1(inputs,batch_size,name='conv'):
    with tf.variable_scope(name):
        ...
        conv0 = tf.layers.conv2d(inputs,filters = 32, kernel_size=(1,1), strides = 1)
        conv1 = tf.layers.conv2d(inputs,filters = 32, kernel_size=(2,1), strides = 1)
        conv2 = tf.layers.conv2d(inputs,filters = 32, kernel_size=(3,1), strides = 1)
        conv3 = tf.layers.conv2d(inputs, filters = 32, kernel_size=(4,1), strides = 1)
        conv4 = tf.layers.conv2d(inputs,filters = 32, kernel_size=(5,1), strides = 1)
        conv5 = tf.layers.conv2d(inputs,filters = 32, kernel_size=(6,1), strides = 1)
        conv6 = tf.layers.conv2d(inputs, filters = 32, kernel_size=(7,1), strides = 1)
        conv7 = tf.layers.conv2d(inputs,filters = 32, kernel_size=(8,1), strides = 1)
        conv8 = tf.layers.conv2d(inputs,filters = 32, kernel_size=(9,1), strides = 1)
        conv9 = tf.layers.conv2d(inputs,filters = 32, kernel_size=(10,1), strides = 1)
        conv10 = tf.layers.conv2d(inputs,filters = 32, kernel_size=(11,1), strides = 1)
        conv11 = tf.layers.conv2d(inputs,filters = 32, kernel_size=(12,1), strides = 1)
        conv12 = tf.layers.conv2d(inputs,filters = 32, kernel_size=(13,1), strides = 1)
        conv13 = tf.layers.conv2d(inputs,filters = 32, kernel_size=(14,1), strides = 1)     
        conv14 = tf.layers.conv2d(inputs,filters = 32, kernel_size=(15,1), strides = 1)
        conv15 = tf.layers.conv2d(inputs,filters = 32, kernel_size=(16,1), strides = 1)   

        conv0 = tf.layers.max_pooling2d(conv0, (397,1), strides = 1)
        conv1 = tf.layers.max_pooling2d(conv1, (396,1), strides = 1)
        conv2 = tf.layers.max_pooling2d(conv2, (395,1), strides = 1)
        conv3 = tf.layers.max_pooling2d(conv3, (394,1), strides = 1)
        conv4 = tf.layers.max_pooling2d(conv4, (393,1), strides = 1)
        conv5 = tf.layers.max_pooling2d(conv5, (392,1), strides = 1)
        conv6 = tf.layers.max_pooling2d(conv6, (391,1), strides = 1)
        conv7 = tf.layers.max_pooling2d(conv7, (390,1), strides = 1)
        conv8 = tf.layers.max_pooling2d(conv8, (389,1), strides = 1)
        conv9 = tf.layers.max_pooling2d(conv9, (388,1), strides = 1)
        conv10 = tf.layers.max_pooling2d(conv10, (387,1), strides = 1)
        conv11 = tf.layers.max_pooling2d(conv11, (386,1), strides = 1)
        conv12 = tf.layers.max_pooling2d(conv12, (385,1), strides = 1)
        conv13 = tf.layers.max_pooling2d(conv13, (384,1), strides = 1)
        conv14 = tf.layers.max_pooling2d(conv14, (383,1), strides = 1)
        conv15 = tf.layers.max_pooling2d(conv15, (382,1), strides = 1)
...
test_support_set_embeddings = encoder(tf.reshape(test_support_set, [n_test_classes * n_test_shot, test_height, test_width, channels]),n_shot = num_shot)
...
test_query_set_embeddings = encoder(tf.reshape(test_query_set, [n_test_classes * n_test_query, img_height, img_width, channels]),n_shot = num_shot)
test_distance = euclidean_distance(test_query_set_embeddings, test_class_prototype)
test_predicted_probability = tf.reshape(tf.nn.log_softmax(-test_distance), [n_test_classes, n_test_query, -1])
...
for epoch in range(num_epochs):
    for episode in range(num_episodes):
        episodic_classes = np.random.permutation([0,1,2,3])[:num_way]
        ...
        for index,class_ in enumerate(episodic_classes):
            selected = np.random.permutation(num_examples[class_])[:num_shot + num_query]
            support[index] = x_train[class_][0,selected[:num_shot]]
            query[index] = x_train[class_][0,selected[num_shot:]]
...
results.to_csv("../results/protonets/v5_3_1_4class_2random.csv",index = False)
