...
num_query = 5 #20
num_examples = 10737 #6553 #per class
num_epochs =  15#20nuz
num_episodes = 300
...
def convolution_block1(inputs,name='conv'):
    with tf.variable_scope(name):
        #here out_channels missing - should they be added somehow?
        conv1 = tf.layers.conv2d(inputs, filters = 100, kernel_size=(3,3),strides = 1) #3,100
        conv = tf.nn.relu(conv1)
        return conv

#encoder is different from protoTensor_original_implementation
def encoder(support_set, reuse=tf.AUTO_REUSE):
    with tf.variable_scope('encoder', reuse = reuse): #possible reuse = Truetf.AUTO_REUSE
        net = convolution_block1(support_set,name='conv_1')
...
support_set = tf.compat.v1.placeholder(tf.float32, [num_way, num_shot, img_height,
    img_width, channels],name = "support_set")
query_set = tf.compat.v1.placeholder(tf.float32, [num_way,num_query, img_height,
    img_width, channels], name = "query_set")
test_support_set = tf.compat.v1.placeholder(tf.float32, [n_test_way, n_test_shot, test_height,
    test_width, channels], name = "test_support_set")
test_query_set = tf.compat.v1.placeholder(tf.float32, [n_test_way, n_test_query, test_height,
    test_width, channels], name = "test_query_set")
test_predicted = tf.compat.v1.placeholder(tf.float32, [n_test_way, n_test_query, test_height,
...
test_y = tf.compat.v1.placeholder(tf.int64, [None, None], name = "test_y")
...
query_set_embeddings = encoder(tf.reshape(query_set, [num_classes * num_query_points, img_height, img_width, channels]), reuse=True)
...
test_query_set_embeddings = encoder(tf.reshape(test_query_set, [num_classes * n_test_query, img_height, img_width, channels]), reuse=True)
...
best_loss = 1000
best_accuracy = 0
train_op = tf.compat.v1.train.AdamOptimizer(learning_rate=0.1).minimize(loss)
...
results.to_csv("../results/protonets/v1.csv",index = False)
