...
def convolution_block1(inputs,batch_size,name='conv'):
    with tf.variable_scope(name):
        ...
        conv1 = tf.layers.conv2d(inputs,filters = 64, kernel_size=(2,1), strides = 1)
        conv2 = tf.layers.conv2d(inputs,filters = 64, kernel_size=(3,1), strides = 1)
        conv3 = tf.layers.conv2d(inputs, filters = 64, kernel_size=(4,1), strides = 1)
        conv4 = tf.layers.conv2d(inputs,filters = 64, kernel_size=(5,1), strides = 1)
        conv5 = tf.layers.conv2d(inputs,filters = 64, kernel_size=(6,1), strides = 1)
        conv6 = tf.layers.conv2d(inputs, filters = 64, kernel_size=(7,1), strides = 1)
        conv7 = tf.layers.conv2d(inputs,filters = 64, kernel_size=(8,1), strides = 1)
        conv8 = tf.layers.conv2d(inputs,filters = 64, kernel_size=(9,1), strides = 1)
...
#loss = tf.reduce_max(0, tf.math.substract(1, tf.multiply(y_one_hot, predicted_probability))
loss = tf.losses.hinge_loss(y_one_hot, predicted_probability)
...
test_loss = tf.losses.hinge_loss(test_y_one_hot, test_predicted_probability)
...
for epoch in range(num_epochs):
    for episode in range(num_episodes):
        ...
        #print(labels)
        _, loss_, accuracy_, predicted_values,  predicted_probability_ = sess.run([train_op, loss, accuracy,predicted, predicted_probability], feed_dict={support_set: support, query_set: query, y:labels})
        #print("pred prob",predicted_probability_)
...
results.to_csv("../results/protonets/v5_3_1_hinge.csv",index = False)
