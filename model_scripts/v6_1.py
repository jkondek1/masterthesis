...
def convolution_block1(inputs,batch_size,name='conv'):
    with tf.variable_scope(name):
        ...
        conv0 = tf.layers.conv2d(inputs,filters = 128, kernel_size=(3,1), strides = 1)
        conv1 = tf.layers.conv2d(inputs,filters = 128, kernel_size=(4,1), strides = 1)
        conv2 = tf.layers.conv2d(inputs,filters = 128, kernel_size=(5,1), strides = 1)
        conv3 = tf.layers.conv2d(inputs, filters = 128, kernel_size=(6,1), strides = 1)
        conv4 = tf.layers.conv2d(inputs,filters = 128, kernel_size=(7,1), strides = 1)
        
        conv0 = tf.layers.conv2d(conv0,filters = 128, kernel_size=(3,100), strides = 1)
        conv1 = tf.layers.conv2d(conv1,filters = 128, kernel_size=(4,100), strides = 1)
        conv2 = tf.layers.conv2d(conv2,filters = 128, kernel_size=(5,100), strides = 1)
        conv3 = tf.layers.conv2d(conv3, filters = 128, kernel_size=(6,100), strides = 1)
        conv4 = tf.layers.conv2d(conv4,filters = 128, kernel_size=(7,100), strides = 1)
...
results.to_csv("../results/protonets/v6_1.csv",index = False)
