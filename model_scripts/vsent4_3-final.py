...
def convolution_block1(inputs,batch_size,name='conv'):
    with tf.variable_scope(name):
        ...
        conv1 = tf.layers.conv2d(inputs,filters = 128, kernel_size=(3,50), strides = 1)
        conv2 = tf.layers.conv2d(inputs,filters = 128, kernel_size=(4,50), strides = 1)
        conv3 = tf.layers.conv2d(inputs, filters = 128, kernel_size=(5,50), strides = 1)
...
test_query_set_embeddings = encoder(tf.reshape(test_query_set, [n_test_classes * n_test_query, test_height, test_width, channels]),
...
results.to_csv("../results/protonets/vsent4_3-final.csv",index = False)
