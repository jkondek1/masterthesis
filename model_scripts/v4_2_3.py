...
num_shot = 100#20 # number of examples per class for support set
num_query = 100 #20
...
def convolution_block1(inputs,batch_size,name='conv'):
    with tf.variable_scope(name):
        ...
        conv1 = tf.layers.conv2d(inputs,filters = 50, kernel_size=(3,1), strides = 1)
        conv2 = tf.layers.conv2d(inputs,filters = 50, kernel_size=(4,1), strides = 1)
        conv3 = tf.layers.conv2d(inputs, filters = 50, kernel_size=(5,1), strides = 1)
...
results.to_csv("../results/protonets/v4_2_3.csv",index = False)
