...
def convolution_block1(inputs,name='conv'):
    with tf.variable_scope(name):
        conv = tf.layers.conv2d(inputs, filters = 100, kernel_size=(3,3)) #3,100
        conv = tf.nn.relu(conv)
        conv = tf.compat.v1.layers.max_pooling2d(conv, 2, strides = 1)
        return conv
...
results.to_csv("../results/protonets/v2.csv",index = False)
