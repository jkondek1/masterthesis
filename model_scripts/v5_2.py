...
def convolution_block1(inputs,batch_size,name='conv'):
    with tf.variable_scope(name):
        ...
        conv1 = tf.layers.conv2d(inputs,filters = 1024, kernel_size=(2,100), strides = 1)
        conv2 = tf.layers.conv2d(inputs,filters = 1024, kernel_size=(3,100), strides = 1)
        conv3 = tf.layers.conv2d(inputs, filters = 1024, kernel_size=(4,100), strides = 1)
        conv4 = tf.layers.conv2d(inputs,filters = 1024, kernel_size=(5,100), strides = 1)
        conv5 = tf.layers.conv2d(inputs,filters = 1024, kernel_size=(6,100), strides = 1)
        conv6 = tf.layers.conv2d(inputs, filters = 1024, kernel_size=(7,100), strides = 1)
        conv7 = tf.layers.conv2d(inputs,filters = 1024, kernel_size=(8,100), strides = 1)
        conv8 = tf.layers.conv2d(inputs,filters = 1024, kernel_size=(9,100), strides = 1)
...
results.to_csv("../results/protonets/v5_2.csv",index = False)
