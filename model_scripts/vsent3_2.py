...
def convolution_block1(inputs,name='conv'):
    with tf.variable_scope(name):
        conv = tf.layers.conv2d(inputs, filters = 128, kernel_size=(15,25)) #3,100
        conv = tf.layers.conv2d(inputs, filters = 128, kernel_size=(10,10))
        conv = tf.nn.relu(conv)
        conv = tf.layers.max_pooling2d(conv, 2, strides = 1)
        return conv
...
results.to_csv("../results/protonets/vsent3_2.csv",index = False)
